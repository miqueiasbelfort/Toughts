const User = require('../models/User')

//Criptografar a senha
const bcrypt = require('bcryptjs')


module.exports = class AuthController {
    
    static login(req, res) {
        res.render('auth/login')
    }

    static async loginPost(req, res){

        const {email, password} = req.body

        // Se usuário existe
        const user = await User.findOne({raw: true, where: {email: email}})

        if(!user){
            req.flash('message', 'Usuário não encontrado!')
            res.render('auth/login')
            return
        }

        //Se a senha combina com a senha do banco
        const passwordMatch = bcrypt.compareSync(password, user.password)
        if(!passwordMatch){
            req.flash('message', 'Senha inválida!')
            res.render('auth/login')
            return
        }

        //inicializar a session
        req.session.userid = user.id

        //message
        req.flash('message', "Login feito com sucesso!")
        
        req.session.save(() => { //salvando a session
            res.redirect('/')
        })

    }

    static register(req, res) {
        res.render('auth/register')
    }
    
    static async registerPost(req, res){

        const {name, email, password, comfirmpassword} = req.body

        // validando a confirmação da senha
        if(password != comfirmpassword){
            //mensagem
            req.flash('message', 'As senhas não conferem, tente novamente!')
            res.render('auth/register')

            return
        }

        //checando se o usuário já existe
        const checkIfUserExists = await User.findOne({raw: true, where: {email: email}})

        if(checkIfUserExists){
            //mensagem
            req.flash('message', 'O e-mail já está em uso!')
            res.render('auth/register')
            return
        }

        //criando e criptografia a senha
        const salt = bcrypt.genSaltSync(10)
        const hashedPassowrd = bcrypt.hashSync(password, salt)

        const user = {
            name,
            email,
            password: hashedPassowrd
        }

        try {
            const createUser = await User.create(user)

            //inicializar a session
            req.session.userid = createUser.id

            //message
            req.flash('message', "Cadastro realizado com sucesso!")
            
            req.session.save(() => { //salvando a session
                res.redirect('/')
            })

        } catch(err) {
            console.log(err)
        }
    }

    static logout(req, res){ //Enserrar a session
        req.session.destroy()
        res.redirect('/login')
    }

}